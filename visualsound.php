...
Description: Allows the insertion of SoundCloud and VisualMix code in Wordpress. ex: <code>[soundcloud playset wildchild seriexl]</code> 
Version: 1.06
...
1.04  - Hidden SoundCloud widget colors added
1.05  - New SoundCloud features added (groups etc..) and fixed Pages bug
1.06  - Groups fix
*/
...
function visualsound_options() {
    ...
    if ( $_POST["visualsound_submit"] ) {
       ...
       $newoptions['commentscolor'] = strip_tags(stripslashes($_POST["commentscolor"]));       
       $newoptions['themecolor'] = strip_tags(stripslashes($_POST["themecolor"]));       
    ...
    if ($options['provider'] == "soundcloud") {
      echo "* <i>track, playtrack, set, playset, group, playgroup, dropbox, cloudplayer</i>";
    ...
    echo '<tr valign="middle"><td><table width="100%">';
    echo '<tr valign="middle"><th scope="row">Comments color</th>';
    echo '<td><input type="text" name="commentscolor" value="'. $options['commentscolor'] . '" size="6"></input><font color="#' . $options['commentscolor'] . '">COLOR</font></td></tr>';
    echo '</table></td><td><table width="100%" cellpadding="0" cellspacing="0"><tr valign="middle"><th scope="row">Theme color</th>';
    echo '<td><input type="text" name="themecolor" value="'. $options['themecolor'] . '" size="6"></input><font color="#' . $options['themecolor'] . '">COLOR</font></td></tr>';
    echo '</table></td></tr>';
    ...
    if ($options['suplugin'] == "true") {
      ...
      echo '<param name="movie" value="http://player.soundcloud.com/player.swf?track=drumplugme-joan-darc-remix&amp;show_comments=' . $options['appletcomments'] . '&amp;auto_play=false&amp;color=' . $options['appletcolor'] . '&amp;comments_color=' . $options['commentscolor'] . '&amp;theme_color=' . $options['themecolor'] . '"></param>';
      echo '<param name="allowscriptaccess" value="always"></param>';
      echo '<param name="wmode" value="transparent"></param>';
      echo '<embed allowscriptaccess="always" height="81" src="http://player.soundcloud.com/player.swf?track=idmix&amp;show_comments=' . $options['appletcomments'] . '&amp;auto_play=false&amp;color=' . $options['appletcolor'] . '&amp;comments_color=' . $options['commentscolor'] . '&amp;theme_color=' . $options['themecolor'] . '" type="application/x-shockwave-flash" width="' . $options['appletwidth'] . '" wmode="transparent"> </embed>';
      echo '</object>';
      echo '<div style="padding-top: 5px;"><a href="http://soundcloud.com/wildchild/idmix" target="_blank">IDMix</a> <span title="' . $visualsound_by . '">by</span>  <a href="http://soundcloud.com/wildchild">wildchild</a> ' . $visualattribution . '</div>';
      echo '</div>';
      echo '<div id="box1"><object height="270" width="' . $options['appletwidth'] . '">';
      echo '<param name="movie" value="http://player.soundcloud.com/player.swf?playlist=seriexl&amp;auto_play=false&amp;color=' . $options['appletcolor'] . '&amp;comments_color=' . $options['commentscolor'] . '&amp;theme_color=' . $options['themecolor'] . '&amp;show_comments=' . $options['appletcomments'] . '&amp;show_artwork=' . $options['appletartwork'] . '&amp;show_playcount=' . $options['appletshowpc'] . '"></param>  <param name="wmode" value="transparent"></param>  <param name="allowscriptaccess" value="always"></param>  <embed allowscriptaccess="always" height="270" src="http://player.soundcloud.com/player.swf?playlist=seriexl&amp;auto_play=false&amp;show_comments=' . $options['appletcomments'] . '&amp;color=' . $options['appletcolor'] . '&amp;show_artwork=' . $options['appletartwork'] . '&amp;comments_color=' . $options['commentscolor'] . '&amp;theme_color=' . $options['themecolor'] . '&amp;show_playcount=' . $options['appletshowpc'] . '" type="application/x-shockwave-flash" width="' . $options['appletwidth'] . '" wmode="transparent"> </embed> </object>';
    ...
    echo ' (links will open in new window)<p><font color="#881111">SoundCloud exposure is encouraged for all users!</font>';
}
